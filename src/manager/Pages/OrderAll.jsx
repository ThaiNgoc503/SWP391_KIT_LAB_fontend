...
const OrderAll = () => {
  ...
  const handleUpdate = async (id) => {
    const data = { deliveryStatus: "Đang giao hàng" };
  ...
  return (
    <div className="flex min-h-screen flex-col gap-10 bg-slate-100 p-2 pb-10">
      <div className="flex justify-end pr-[6rem]">
        <ul ref={menuRef} className="inline-flex justify-end gap-2 pt-10">
          ...
          <li className="relative">
            <button
              onClick={() => setMenuUpdate(!menuUpdate)}
              className="rounded-md bg-red-200 p-2"
            >
              Order
            </button>
            {menuUpdate && (
              <ul className="absolute -left-10 mt-1 flex w-[10rem] flex-col rounded-md bg-white">
                <li className="rounded-md border-b-[1px] border-slate-300 px-2 py-1 hover:bg-slate-200">
                  All
                </li>
                <li className="rounded-md border-b-[1px] border-slate-300 px-2 py-1 hover:bg-slate-200">
                ...
                <li className="rounded-md border-b-[1px] border-slate-300 px-2 py-1 hover:bg-slate-200">
                  <Link to="/manager/delivery">
                    <p>Delivery</p>
                  </Link>
                </li>
                <li className="rounded-md border-b-[1px] border-slate-300 px-2 py-1 hover:bg-slate-200">
                  <Link to="/manager/delivery-success">
          ...
          <table className="max-w-screen mx-3 text-left text-base text-black">
            ...
            <tbody>
              {filteredOrder.filter(
                (order) => order.deliveryStatus === "Đã đặt hàng",
              ).length == 0 ? (
                <tr>
                ...
                filteredOrder
                  .filter((order) => order.deliveryStatus === "Đã đặt hàng")
                  .map((order, index) => (
